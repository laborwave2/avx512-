...
static void limbs18_to_mpz_base29_top12(mpz_t rop, const uint64_t limbs[18]) {
...
static bool check_ranges_limbs18_base29_top12(const uint64_t limbs[18]) {
    for (int i = 0; i < 17; ++i) {
...
bool run_tests_8_1_avx512F(std::size_t iterations, std::uint64_t seed) {
    if (!run_known_cases()) return false;
    if (!run_random(iterations, static_cast<uint64_t>(seed))) return false;
    std::cout << "[OK] 8_1/avx512F tests passed: iterations=" << iterations << " lanes/iter=8\n";
    return true;
}
