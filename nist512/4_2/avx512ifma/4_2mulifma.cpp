...
void mod4_2(__m512i in[15], __m512i out[5], const __m512i P[5]){
   for (int i = 0; i < 10; i++) {
      ...
      in[i + 1] = _mm512_add_epi64(in[i + 1], C);
      ...
      in[i + 5] = _mm512_mask_add_epi64(in[i + 5], 0x55, in[i + 5],
                                        _mm512_shuffle_epi32(in[i], static_cast<_MM_PERM_ENUM>(0x4E)));
   }
   ...
   for (int pass = 0; pass < 2; pass++) {
      ...
      out[0] = _mm512_mask_add_epi64(out[0], 0xAA, out[0],
                                     _mm512_shuffle_epi32(C, static_cast<_MM_PERM_ENUM>(0x4E)));
   }
}
