...
void bench_vs_gmp_4_2_avx512ifma(size_t N = 2000, uint64_t seed = 42);
...
void bench_cycles_512(const char* name, F&& f, const std::vector<Pair512>& vec, int rounds=100 )
{
    ...
    for (int i = 0; i < 1000; ++i) {
        ...
        f(const_cast<__m512i*>(p.a), const_cast<__m512i*>(p.b), out, P);
    ...
    for (int r = 0; r < rounds; ++r) {
        ...
        for (size_t i = 0; i < N; ++i) {
            f(const_cast<__m512i*>(vec[i].a), const_cast<__m512i*>(vec[i].b), out, P);
            alignas(64) uint64_t tmp[8];
            _mm512_store_si512((void*)tmp, out[0]);
            sink ^= tmp[0];
        }
